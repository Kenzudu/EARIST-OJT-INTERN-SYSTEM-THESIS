...
function CoordinatorMessages() {
    ...
    const [replyingTo, setReplyingTo] = useState(null); // Message dealing with reply
    const [hoveredMessageId, setHoveredMessageId] = useState(null); // For showing reply button
    const [isTyping, setIsTyping] = useState(false); // Typing indicator state
    const typingTimeoutRef = useRef(null); // Ref for typing debounce
    ...
    const fetchMessages = async () => {
        try {
            ...
            const res = await axios.get(`${baseURL}coordinator/messages/`, {
            ...
            if (err.response?.status === 403) {
                setError('Access denied. Please check if you have coordinator permissions.');
    ...
    const fetchUsers = async () => {
        try {
            ...
            // Filter to show admins and supervisors
    ...
    const handleSendMessage = async (e) => {
        ...
        try {
            ...
            await axios.post(`${baseURL}coordinator/messages/`, formData, {
                headers: {
    ...
    const sendTypingStatus = async (isTypingStatus) => {
        try {
            ...
            await axios.post(`${baseURL}typing/update/`,
                { recipient_id: chatUser.id, is_typing: isTypingStatus },
                { headers: { Authorization: `Token ${token}` } }
            );
        } catch (err) {
    ...
    const handleSendChatMessage = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${baseURL}coordinator/messages/`, formData, {
                headers: {
                    Authorization: `Token ${token}`,
                    'Content-Type': 'multipart/form-data'
                }
            ...
            const newMessage = {
                ...response.data,
                // Fallbacks for immediate UI update (handles stale backend or partial response):
                type: 'sent',
    ...
    const markAsRead = async (messageId) => {
        try {
            const token = localStorage.getItem('token');
            await axios.put(`${baseURL}coordinator/messages/${messageId}/read/`, {}, {
                headers: { Authorization: `Token ${token}` }
            });
            fetchMessages();
        } catch (err) {
    ...
    const handleDeleteMessage = async (messageId, e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`${baseURL}coordinator/messages/${messageId}/`, {
                headers: { Authorization: `Token ${token}` }
            });
    ...
    const handleDeleteAll = async () => {
        ...
        try {
            ...
            const response = await axios.post(`${baseURL}coordinator/messages/delete-all/`, {}, {
                headers: { Authorization: `Token ${token}` }
            });
    ...
    const getUniqueConversations = () => {
        ...
        messages.forEach(msg => {
            ...
            if (!conversationMap.has(contactId)) {
                ...
                // Try to get userId from users list first, then fallback to extracting from message
                let userId = user ? user.id : null;

                // If userId is still null, try to extract it from the message recipient_id or sender_id
                if (!userId) {
                    // For sent messages, we don't have recipient_id in the response
                    // For received messages, we can try to find it from other messages
                    const relatedMsg = messages.find(m =>
                        (m.sender === contactId || m.recipient === contactId)
                    );
                    // Since we don't have direct access to user IDs in messages, use contactId as fallback
                    userId = contactId; // Use username as fallback
                }
    ...
    const handleDeleteConversation = async () => {
        ...
        try {
            ...
            const deletePromises = chatMessages.map(msg =>
                axios.delete(`${baseURL}coordinator/messages/${msg.id}/`, {
                    headers: { Authorization: `Token ${token}` }
                })
            );
    ...
    return (
        <div className="admin-dashboard-container">
            ...
            <div className="admin-dashboard-main">
                <CoordinatorHeader
                    title="Messages"
                    subtitle="Communicate with administrators and supervisors"
                />
                    ...
                    <div className="messages-container">
                        {loading ? (
                            ...
                            <div className="empty-state">
                                ...
                                <p>Start a conversation with administrators or supervisors</p>
                            </div>
                        ) : (
