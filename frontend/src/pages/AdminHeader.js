// AdminHeader component – displays page title, subtitle, logged‑in admin info, notifications, and profile picture
...
function AdminHeader({ title, subtitle }) {
    ...
    const displayName = (() => {
        ...
        if (typeof currentAdmin === "string") return currentAdmin;
        const full =
            currentAdmin.full_name ||
            `${currentAdmin.first_name || ""} ${currentAdmin.last_name || ""}`.trim();
        return full || currentAdmin.username || "Admin";
    })();
    ...
    // Load profile picture and admin info
    useEffect(() => {
        const fetchProfile = async () => {
            try {
                ...
                // Fetch user profile using correct endpoint
                ...
                if (currentAdmin) {
                    const name = currentAdmin.full_name ||
                        `${currentAdmin.first_name || ''} ${currentAdmin.last_name || ''}`.trim();
                    if (name) setFullName(name);
                }
            }
    ...
    useEffect(() => {
        const fetchMessageNotifications = async () => {
            try {
                ...
                const savedRead = localStorage.getItem('adminReadNotifications');
        ...
        const interval = setInterval(fetchMessageNotifications, 30000); // Check every 30 seconds
    ...
    return (
        <header className="student-dashboard__header">
            <div>
                ...
                <p>{subtitle || "Manage users, roles, and system configuration"}</p>
            </div>
            <div className="student-dashboard__welcome">
                ...
                {/* Profile Picture – click goes to admin settings */}
                <div className="header-profile-picture" onClick={() => navigate('/admin/settings')} style={{ cursor: 'pointer' }}>
                    <img
