...
print(f"After removing null bytes: {len(content_clean)} bytes")

# Convert to string
try:
    content_str = content_clean.decode('utf-8')
except UnicodeDecodeError:
    print("UTF-8 decode failed, trying latin-1...")
    content_str = content_clean.decode('latin-1')

# Find the last proper closing of a function (before corruption)
# Look for the document_templates_list function end
marker = "        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)"

last_marker_pos = content_str.rfind(marker)
...
end_of_line = content_str.find('\n', last_marker_pos)
if end_of_line == -1:
    end_of_line = len(content_str)

# Cut everything after this point
clean_content = content_str[:end_of_line + 1]

print(f"Truncated to: {len(clean_content)} bytes")

# Now add the two new functions
new_functions = """
...
final_content = clean_content + new_functions

print(f"Final size: {len(final_content)} bytes")

# Write the fixed file
with open('core/views.py', 'w', encoding='utf-8', newline='\n') as f:
